...
int followThrough;
int slapDirection1{DEG180_SLAP}, slapDirection2{DEG0_SLAP}, restingDirection{DEG90_SLAP};
int decideMove(const std::bitset<5>& sensorReadings);
void getDelta();
void getMove();
void (*deltaPtr)(){&getDelta};
void (*movePtr)(){&getMove};
// decides all the moves based on the sensor readings
Task movement(0, TASK_FOREVER, movePtr, &ts, true);
Task servoTurn (20*TASK_MILLISECOND, TASK_FOREVER, deltaPtr, &ts, true);
...
int decideMove(const std::bitset<5>& sensorReadings){
    ...
    switch (data) {
        ...
        case 0: {
            ...
            while (elapsed < 150) {
                servoTurn.disable();
                std::uint16_t delta2 = servoSignal(slapDirection1);
                robotStop();
                delayMicroseconds(delta2);
                delta2 = servoSignal(slapDirection2);
                robotStop();
                delayMicroseconds(delta2);
                elapsed++;
            }
    ...
    if (data != 0)
        previousData = data;
//    delayMicroseconds(delta); Removed this
    return data;
}
